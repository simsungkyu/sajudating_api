...
# Task plan: itemNcard next steps (2026-02-01T073817)
...
**Automated execution** (repo root): `bash docs/saju/itemNcard/run_verification.sh` was run. **Result**: 통과. `make test-itemncard` (types/itemncard, service/itemncard, service, mcplocal) and `go build .` both succeeded.

| Task | Status | What was done |
|------|--------|----------------|
| **1** | Pending (manual) | PRD §7 checklist (1)–(5): requires verification 담당자 to run admweb and record in Verification.md. Not executed in this run. |
| **2** | Pending (manual) | Full-flow check (saju 인생/연도별/월별, pair): requires manual UI verification. Not executed in this run. |
| **3** | Pending (manual) | Seed-folder linkage: requires "시드에서 불러오기" → 등록 → 추출 test in admweb. Not executed in this run. |
| **4** | Done | Ran `make test-itemncard` via run_verification.sh; reviewed TestScope.md. Coverage confirmed: types/itemncard, service/itemncard, service, mcplocal. No gap identified; no new test or doc added. Verification.md 검증 이력에 "테스트 로직·범위 확인" 반영된 행 추가함. |
| **5** | 해당 없음 | No issues recorded from tasks 1–2 in this run; skip. |
| **6** | Done | run_verification.sh executed; make test-itemncard + go build 성공. Verification.md 검증 이력에 2026-02-01 행 추가: task_2026-02-01T073817, 통과, Task 4·Task 6 반영. |
| **7–9** | Optional / skipped | LLM preview E2E, MCP E2E, PRD §6 final check: not executed in this run. |

**Remaining / next steps**

- **Tasks 1–3**: Verification 담당자가 admweb에서 §7 체크리스트(1)–(5), 전체 흐름(사주 인생·연도별·월별, 궁합), 시드 연동을 1회 수행 후 Verification.md 검증 이력에 항목별 결과 기록.
- **Tasks 7–9**: Optional; 수행 시 OperatorGuide·Verification·runLoopForSaju.sh 등 문서 보완.
