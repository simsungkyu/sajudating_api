...
# Task plan: itemNcard next steps (2026-02-01T055919)
...
- **Task 5 (릴리스 전 자동 검증)**: repo root에서 `bash docs/saju/itemNcard/run_verification.sh` 실행. `make test-itemncard` 및 `go build .` 모두 통과. Verification.md 검증 이력 테이블에 행 추가: `| 2026-02-01 | (task 실행) | 통과 | task_2026-02-01T055919: run_verification.sh 실행. make test-itemncard + go build 성공. §7·전체 흐름 수동 점검 대기. |`

**Status**

- Task 1·2: 수행 안 함. 검증 담당자 수동 점검 필요.
- Task 3: §7·전체 흐름 결과는 Task 1·2 수행 후 기록. 자동 검증(Task 5) 결과 행만 이번 실행으로 추가됨.
- Task 4: 해당 없음 (발견 이슈 없음).
- Task 5: 완료. 통과.
- Task 6·7: 미수행 (선택 항목).

**Remaining / next steps**

1. 검증 담당자: Task 1 (PRD §7 체크리스트 (1)–(5) 1회), Task 2 (전체 흐름 점검) 수행 후 메모.
2. Task 3: 위 결과를 Verification.md 검증 이력에 항목별 기록(§7 (1)–(5), 전체 흐름 사주 인생·연도별·월별·궁합) 추가.
3. (선택) Task 6: LLM 컨텍스트 미리보기 E2E 수동 확인.
4. (선택) Task 7: y2sl-local MCP E2E 확인.
