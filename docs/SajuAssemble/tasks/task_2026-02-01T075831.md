# Task plan: itemNcard next steps (2026-02-01T075831)
...
- **Previous plan (task_2026-02-01T075518)**: Code verification and TestScope/Verification.md updates done; manual verification and local automated run left to operator.
...
- **Task 4**: Automated verification run from repo root. Executed `go test ./types/itemncard/... ./service/itemncard/... ./service/... ./mcplocal/... -count=1` and `go build .` in `api/` (via `bash -c`; `make test-itemncard` equivalent). **Result**: All tests passed (types/itemncard, service/itemncard, service, mcplocal). Build succeeded. One row added to Verification.md 검증 이력 (task_2026-02-01T075831).
- **Task 6**: Test scope review. Same test run as Task 4. TestScope.md coverage (types/itemncard, service/itemncard, AdminExtractService, mcplocal) and flow/scope alignment confirmed; no gap found. No new test or doc update required. Reflected in the same Verification.md row.
- **Task 7**: Verification.md 검증 이력 반영. One row added consolidating Task 4 and Task 6 results (날짜 2026-02-01, 결과 통과, 비고에 task_2026-02-01T075831·테스트 로직·범위 확인 명시).

**Skipped / N/A**

- **Task 1**: PRD §7 checklist (1)–(5). Manual check in admweb; not executed in this run. **Remaining**: 검증 담당자가 Verification.md §7 체크리스트 5항목 1회 수행 후 검증 이력에 기록.
- **Task 2**: Full-flow check (saju 인생/연도별/월별, pair). Manual check; not executed. **Remaining**: 검증 담당자가 Verification.md 전체 흐름 점검 1회 수행 후 검증 이력에 기록.
- **Task 3**: Seed–extraction linkage. Manual check in admweb; not executed. **Remaining**: 시드에서 불러오기 → 일괄 등록 → 추출 테스트로 시드 카드 선택 확인 후 검증 이력에 "seed 연동 확인 완료" 또는 이슈 1줄 기록.
- **Task 5**: Conditional fix. No issues recorded from tasks 1–3 in this run. **Status**: 해당 없음.
- **Tasks 8–10**: Optional (LLM context preview E2E, MCP E2E, PRD §6 final check). Not executed. **Remaining**: 필요 시 검증 담당자가 수동 점검 및 문서 보완.

**Status**: Task 4, 6, 7 완료. Task 1, 2, 3 수동 점검 및 선택 항목(8–10)은 검증 담당자 수행 대기.

**Next steps**: (1) 검증 담당자: §7 체크리스트 (1)–(5), 전체 흐름 점검, seed 연동 확인을 admweb에서 1회 수행하고 Verification.md 검증 이력에 항목별 결과 추가. (2) 선택: Task 8–10(LLM 미리보기, MCP E2E, PRD §6 최종 대조) 수행.
