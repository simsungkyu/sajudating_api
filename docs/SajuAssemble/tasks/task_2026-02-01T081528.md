# Task plan: itemNcard next steps (2026-02-01T081528)
...
- **Task 1**: Verification.md 검증 이력에 §7 체크리스트 (1)~(5) 수동 수행 대기 행 추가. (검증 담당자 1회 실행 후 항목별 결과 기록 대기.)
- **Task 2**: Verification.md에 전체 흐름 점검(사주 인생/연도별/월별, 궁합) 수동 수행 대기 행 추가.
- **Task 3**: Verification.md에 seed 연동 수동 확인 대기 행 추가.
- **Task 4**: Repo root에서 `bash docs/saju/itemNcard/run_verification.sh` 실행. **통과**: `make test-itemncard` 및 `go build .` 성공. types/itemncard, service/itemncard, service, mcplocal 패키지 통과. Verification.md 검증 이력에 Task 4 행 추가.
- **Task 5**: 해당 없음 (Tasks 1–3에서 기록된 이슈 없음).
- **Task 6**: `make test-itemncard` 실행( run_verification.sh 내). TestScope.md 대조(types/itemncard, service/itemncard, service, mcplocal). 갭 없음. Verification.md에 "테스트 로직·범위 확인 완료" 행 추가.
- **Task 7**: Verification.md 검증 이력에 Task 1–4·6 결과를 반영한 통합 행 추가. §7·전체 흐름·seed 수동 점검 대기, Task 5 해당 없음, Tasks 8~10 선택으로 기록.

### Status

| Task | Status |
|------|--------|
| 1–3 | 완료 (Verification.md 행 추가; 수동 점검은 검증 담당자 수행 대기) |
| 4 | 완료 (자동 검증 통과) |
| 5 | 해당 없음 |
| 6 | 완료 (테스트 범위 확인, 갭 없음) |
| 7 | 완료 (검증 이력 통합) |
...
- **검증 담당자**: §7 체크리스트 (1)~(5) 1회 수행 후 Verification.md에 항목별 통과/이슈 기록.
- **검증 담당자**: 전체 흐름 점검(사주 인생·연도별·월별, 궁합) 1회 수행 후 이력 기록.
- **검증 담당자**: 시드에서 불러오기 → 일괄 등록 → 추출 테스트로 시드 카드 선택 확인 후 "seed 연동 확인 완료" 또는 이슈 1줄 기록.
- (선택) Task 8: LLM context preview E2E 및 OperatorGuide 확인.
- (선택) Task 9: y2sl-local MCP E2E 및 runLoopForSaju.sh 문서 확인.
- (선택) Task 10: PRD §6 성공 기준 최종 점검.
