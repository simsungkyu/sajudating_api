...
# Task plan: itemNcard next steps (2026-02-01T071810)
...
| Task | Status | Notes |
|------|--------|--------|
| **1. PRD §7 체크리스트 (1)–(5)** | 수동 점검 대기 | 검증 담당자가 admweb에서 5항목 1회 수행 후 Verification.md §7 체크박스 및 검증 이력 테이블에 항목별 결과 기록 필요. 에이전트 비수행. |
| **2. 전체 흐름 점검** | 수동 점검 대기 | 사주 인생/연도별/월별, 궁합 추출 테스트 전체 흐름 1회 수행 및 검증 이력 기록. 에이전트 비수행. |
| **3. seed 연동 확인** | 수동 점검 대기 | 시드에서 불러오기 → 일괄 등록 후 추출 테스트로 카드 선택 반영 여부 확인. 에이전트 비수행. |
| **4. 검증 발견 이슈 1건 수정** | 해당 없음 | Task 1·2 미수행으로 발견 이슈 없음. |
| **5. 로컬 자동 검증** | 완료 | repo root에서 `bash docs/saju/itemNcard/run_verification.sh` 실행: **통과**. make test-itemncard (types/itemncard, service/itemncard, service, mcplocal) 및 go build . 성공. Verification.md 검증 이력에 1행 추가함. |
| **6. (선택) LLM 미리보기 E2E** | 미실행 | 선택 항목. |
| **7. (선택) MCP E2E·runLoopForSaju.sh** | 미실행 | 선택 항목. |
| **8. (선택) PRD §6 최종 체크** | 미실행 | 선택 항목. |

**Next steps**: 검증 담당자가 Task 1·2·3 수동 점검 수행 후 Verification.md §7 체크리스트 체크 및 검증 이력 테이블에 §7 (1)–(5) 항목별·전체 흐름·seed 연동 결과 기록. 필요 시 Task 4(이슈 1건 수정) 수행. 선택 시 Task 6–8 수행.
