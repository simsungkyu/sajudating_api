...
# Task plan: itemNcard next steps (2026-02-01T073437)
...
- **Work**: Execute Verification.md **§7 checklist** in order: (1) 사주 추출 결과화면 — 원국 음양 볼드·오행 컬러 표시, (2) 궁합 추출 결과화면 — A/B 명식 분석 정보 요약, (3) seed 폴더 참조·시드에서 불러오기·일괄 등록, (4) 사주 추출 — LLM 요청 전체 내용 textarea, (5) 궁합 추출 — LLM 요청 전체 내용 textarea. For each item note pass/fail and one-line finding. Add one row (or one row per item) to Verification.md **검증 이력**.
...
- **Task 3 (doc)**: Seed.md "Seed 파일 참조 현황" and `docs/saju/itemNcard/seed/` list aligned. Nine seed options and "일괄 등록" are documented. Added one row to Verification.md 검증 이력: "seed 폴더 참조 확인" (수동 시드 연동 점검 대기).
- **Task 4**: From repo root ran `cd api && make test-itemncard`. All packages passed (types/itemncard, service/itemncard, service, mcplocal). TestScope.md reviewed; coverage (validation, itemncard pipeline, AdminExtractService, mcplocal) confirmed. No new test or gap doc added; existing TestScope.md and Verification.md history already record "테스트 로직·범위 확인 완료". Added one row to Verification.md for this run.
- **Task 5**: 해당 없음. No verification findings from tasks 1–2 to fix (tasks 1–2 not executed in this run).
- **Task 6**: From repo root ran `cd api && make test-itemncard && go build .` (bash -c). Result: **통과**. All tests passed; `go build .` succeeded (third-party go-m1cpu warnings only). Added one row to Verification.md 검증 이력.

**Not executed (manual or optional)**

- **Task 1**: PRD §7 checklist (1)–(5) — requires verification 담당자 to run admweb and record item-level results in Verification.md.
- **Task 2**: Full-flow check (saju 인생/연도별/월별, pair) — manual; same as above.
- **Tasks 7–9**: Optional (LLM preview E2E, MCP E2E·runLoopForSaju.sh, PRD §6 final check); skipped.

**Status**

- Automated verification: **완료** (make test-itemncard + go build . 성공).
- Manual verification: **대기** (§7 checklist (1)–(5), 전체 흐름, seed 연동 수동 확인).

**Next steps**

1. 검증 담당자: Run Tasks 1–2 (PRD §7 checklist, full-flow); record results in Verification.md 검증 이력.
2. Optionally run Tasks 7–9 when resources allow.
