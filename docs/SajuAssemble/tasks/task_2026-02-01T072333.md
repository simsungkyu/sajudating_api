...
# Task plan: itemNcard next steps (2026-02-01T072333)
...
### 1. PRD §7 checklist (1)–(5): one-time run and record in Verification.md
...
### 3. Seed-folder linkage: confirm seed files affect extraction results (PRD §7 (3))
...
### 5. Re-run local automated verification and update Verification.md
...
**Summary**: Automated verification (Task 5) completed; tests and build pass. Tasks 1–3 require manual execution by verification owner. Task 4 skipped (no issues). Optional tasks 6–8 not executed.

| Task | Status | What was done |
|------|--------|----------------|
| **1. PRD §7 checklist (1)–(5)** | Pending (manual) | Scope: manual check + Verification.md update. Not executed by agent; verification owner should run Verification.md §7 checklist and add item-level results to 검증 이력. |
| **2. Full-flow check** | Pending (manual) | Scope: manual check + Verification.md update. Not executed; verification owner should run 전체 흐름 점검 (사주 인생·연도별·월별, 궁합) and add one row to 검증 이력. |
| **3. Seed-folder linkage** | Doc aligned | Seed.md "Seed 파일 참조 현황" already matches pattern: 9 options in admweb (seedCards.ts) ↔ seed/ files; seed/ has many more JSON files loadable via 일괄 등록. Manual confirmation that extraction uses seeded cards (시드에서 불러오기 → bulk-register → extraction test) and adding "seed 연동 확인 완료" to Verification.md remains for verification owner. |
| **4. Fix one verification finding** | 해당 없음 | No issues recorded from tasks 1–2 in this run; skipped. |
| **5. Re-run automated verification** | Done | From repo root: `go -C api test ./types/itemncard/... ./service/itemncard/... ./service/... ./mcplocal/... -count=1` and `go -C api build .` — both passed. Added one row to Verification.md 검증 이력 (task_2026-02-01T072333, 통과). Note: In this environment `cd api && make test-itemncard` did not produce captured output; `go -C api` from repo root succeeded. |
| **6. (Optional) LLM preview E2E** | Not executed | Optional; manual check + doc. |
| **7. (Optional) MCP E2E·runLoopForSaju.sh** | Not executed | Optional; manual check + doc. |
| **8. (Optional) PRD §6 final check** | Not executed | Optional; doc comparison + one row if needed. |

**Remaining / next steps**

1. Verification owner: Run Task 1 (PRD §7 checklist (1)–(5)) and Task 2 (full-flow check), then add results to Verification.md 검증 이력 (item-level or one row per flow).
2. Verification owner: Optionally run Task 3 manual step (시드에서 불러오기 → bulk-register → extraction test → confirm selected cards include seeded cards); if confirmed, add "seed 연동 확인 완료" to Verification.md.
3. Before release: Re-run automated verification from repo root (`cd api && make test-itemncard && go build .` or `go -C api test ... && go -C api build .`). Ensure Verification.md has a recent 통과 row.
4. Optional: Tasks 6–8 when resources allow.
