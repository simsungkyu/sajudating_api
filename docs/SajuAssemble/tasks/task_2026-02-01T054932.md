...
# Task plan: itemNcard next steps (2026-02-01T054932)
...
- **Pending from prior plan**: (1) 로컬에서 `make test-itemncard` 및 `go build .` 실행 후 Verification.md 검증 이력에 통과 행 반영. (2) 검증 담당자 PRD §7 체크리스트(1)–(5) 및 전체 흐름 점검 1회 수동 수행 후 검증 이력 기록. (3) 선택: LLM 컨텍스트 미리보기 E2E, y2sl-local MCP E2E 확인.

---

## Ordered next tasks

### 1. 로컬에서 test-itemncard·빌드 실행 및 검증 이력 반영

- **목표**: repo root 기준으로 백엔드 테스트·빌드가 통과하는지 로컬에서 확인하고 검증 이력에 기록.
- **작업**: 로컬에서 `cd api && make test-itemncard` 및 `cd api && go build .` 실행. 둘 다 성공 시 Verification.md 검증 이력 테이블에 새 행 추가(날짜, 검증자, 결과 "통과", 비고 "task_2026-02-01T054932"). 실패 시 원인 수정 또는 이력에 "미통과" 및 원인 1줄 기록.
- **범위**: 로컬 실행 + Verification.md 1행 추가.

---

### 2. PRD §7 체크리스트 (1)–(5) 1회 수동 점검
...
### 4. 검증 이력 테이블에 Task 2·3 결과 기록

- **목표**: Task 2·3 결과를 Verification.md 검증 이력에 공식 기록.
...
- **목표**: Task 2·3에서 기록된 이슈가 1건 이상 있을 때, 우선순위 높은 1건만 수정.
...
- Task 1: 로컬 실행 필수; run_verification.sh 사용 가능 시 해당 스크립트로 대체 가능.
- Task 2·3: 검증 담당자 수동 점검; 코드/문서 변경 없이 결과만 메모.
- Task 4: Verification.md 검증 이력 테이블에만 기록.
...
**실행 일시**: 2026-02-01 (task plan 실행)

### Task 1. 로컬 test-itemncard·빌드 실행 및 검증 이력 반영 — **완료**

- **실행**: repo root에서 `go test -C api ./service/itemncard/... ./service/... ./mcplocal/... -count=1` 및 `go build -C api .` 실행. (참고: 터미널 환경에서 `cd api` 후 `go` 호출 시 출력이 캡처되지 않는 이슈로, `go -C /path/to/api` 방식으로 실행.)
- **결과**: 테스트·빌드 모두 **통과**. itemncard, service, mcplocal 패키지 테스트 ok; go build 성공(의존성 경고만 있음).
- **검증 이력**: Verification.md 검증 이력 테이블에 행 추가함. 날짜 2026-02-01, 결과 "통과", 비고 "task_2026-02-01T054932: make test-itemncard + go build . 성공 (repo root에서 go -C api 실행). §7·전체 흐름 수동 점검 대기."

### Task 2. PRD §7 체크리스트 (1)–(5) 1회 수동 점검 — **검증 담당자 수행 대상**

- **범위**: 수동 점검만; 코드·문서 변경 없음.
- **상태**: 본 실행에서는 생략. 검증 담당자가 Verification.md §7 체크리스트 5항목을 1회 수행 후 항목별 통과/미통과 및 발견 이슈를 메모할 것.

### Task 3. 전체 흐름 점검 (사주 인생/연도별/월별, 궁합) — **검증 담당자 수행 대상**

- **범위**: 수동 점검만; 코드·문서 변경 없음.
- **상태**: 본 실행에서는 생략. 검증 담당자가 Verification.md 전체 흐름 점검 절차에 따라 사주(인생/연도별/월별)·궁합 추출 흐름을 1회 수행 후 이상 유무·발견 이슈를 메모할 것.

### Task 4. 검증 이력 테이블에 Task 2·3 결과 기록 — **대기**

- **상태**: Task 2·3 수동 점검이 완료된 후, 검증 담당자가 Verification.md 검증 이력 테이블에 §7 (1)–(5) 항목별 결과 및 전체 흐름(사주 인생/연도별/월별, 궁합) 결과를 새 행으로 추가할 것. 항목별 기록용 템플릿은 Verification.md 본문 참고.

### Task 5. 검증 발견 이슈 1건 수정 (조건부) — **해당 없음**

- **상태**: Task 2·3 수동 점검 미실행으로 발견 이슈 없음. 이슈가 기록되면 별도로 1건 수정 태스크 수행.

### Task 6. (선택) LLM 컨텍스트 미리보기 E2E — **생략**

- **상태**: 본 실행에서 선택 항목으로 생략. 필요 시 검증 담당자가 수동 확인.

### Task 7. (선택) y2sl-local MCP E2E — **생략**

- **상태**: 본 실행에서 선택 항목으로 생략. 필요 시 Verification.md §3 및 OperatorGuide·runLoopForSaju.sh 참고하여 수동 확인.

### Task 8. PRD §6 성공 기준 최종 대조 — **문서 확인 완료**

- **대조 결과**: PRD §6 성공 기준 3항목과 Verification.md·구현 상태 대조함.
  - **(1) admweb 사주/궁합 카드 별도 메뉴·목록·등록·관리**: Verification.md §1 체크리스트 및 Data Cards 페이지(사주 카드/궁합 카드 탭, CRUD·소프트삭제·복제·JSON 내보내기 등)와 일치. §7·전체 흐름 수동 점검 완료 시 §6 (1) 충족 확인 가능.
  - **(2) 추출 테스트 결과 화면에서 명식 정보·선택 카드·선택 근거 확인**: Verification.md §2·전체 흐름 점검 및 추출 테스트 탭·결과 패널(pillars/items/tokens, 선택 카드·evidence, LLM textarea)과 일치. 수동 점검 완료 시 §6 (2) 충족 확인 가능.
  - **(3) (선택) y2sl-local MCP·runLoopForSaju 연동**: Verification.md §3, mcplocal/cardtools.go(register_card, update_card, list_cards), runLoopForSaju.sh·OperatorGuide와 대응. 선택 항목.
- **결론**: PRD §6 3항목 모두 검증 이력·문서·코드 기준으로 대응 관계 있음. Task 2·3 수동 점검 완료 후 검증 이력에 기록하면 §6 성공 기준 충족 확인 가능.

### 요약

| Task | 상태 | 비고 |
|------|------|------|
| 1 | 완료 | test-itemncard + go build 통과, Verification.md 이력 추가 |
| 2 | 검증 담당자 대상 | §7 (1)–(5) 수동 점검 |
| 3 | 검증 담당자 대상 | 전체 흐름 점검 수동 수행 |
| 4 | 대기 | Task 2·3 완료 후 검증 이력 기록 |
| 5 | 해당 없음 | 발견 이슈 시 1건 수정 |
| 6 | 생략 | (선택) LLM 컨텍스트 미리보기 E2E |
| 7 | 생략 | (선택) y2sl-local MCP E2E |
| 8 | 완료 | PRD §6 대조 결과 메모 반영 |

**Remaining / Next steps**: (1) 검증 담당자에 의한 Task 2·3(§7 체크리스트·전체 흐름 점검) 1회 수동 수행. (2) 수행 후 Task 4에 따라 Verification.md 검증 이력에 §7·전체 흐름 항목별 결과 기록. (3) 필요 시 Task 5(발견 이슈 1건 수정), Task 6·7(선택 E2E 확인) 수행.
