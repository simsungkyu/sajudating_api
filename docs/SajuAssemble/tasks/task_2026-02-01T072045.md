...
# Task plan: itemNcard next steps (2026-02-01T072045)
...
**Summary**: Task 5 (로컬 자동 검증) 실행 완료. Task 1–3은 수동 점검 필요. Task 4 해당 없음. Task 6–8 선택 항목 미실행.

| Task | Status | Notes |
|------|--------|--------|
| 1. PRD §7 체크리스트 (1)–(5) | **대기** | 검증 담당자 수동 수행 후 Verification.md §7 항목별 결과 및 검증 이력 기록 필요. |
| 2. 전체 흐름 점검 | **대기** | 검증 담당자 수동 수행 후 Verification.md 전체 흐름(사주 인생/연도별/월별, 궁합) 결과 기록 필요. |
| 3. seed 연동 확인 | **대기** | 수동 확인(시드 불러오기 → 일괄 등록 → 추출 테스트로 카드 선택 여부) 후 이력 또는 이슈 1줄 기록. |
| 4. 검증 발견 이슈 1건 수정 | **해당 없음** | Task 1·2 이력에 기록된 이슈 없음; 이슈 발견 시 별도 수정. |
| 5. 로컬 자동 검증 | **완료** | `bash -c 'cd api && make test-itemncard && go build .'` 성공. types/itemncard, service/itemncard, service, mcplocal 통과. Verification.md 검증 이력에 1행 추가함. |
| 6–8. (선택) | **미실행** | LLM 미리보기 E2E, MCP E2E·runLoopForSaju.sh, PRD §6 최종 체크 — 리소스 허용 시 수행. |

**Done**:
- Repo root 기준 `cd api && make test-itemncard && go build .` 실행( bash -c 사용 ). 테스트·빌드 통과.
- Verification.md 검증 이력 테이블에 2026-02-01 task_2026-02-01T072045 실행 결과 1행 추가.

**Remaining / Next steps**:
1. 검증 담당자: Task 1 (§7 체크리스트 5항목) 1회 수행 → Verification.md §7 체크 및 검증 이력 항목별 기록.
2. 검증 담당자: Task 2 (전체 흐름 점검) 1회 수행 → Verification.md 전체 흐름 결과 기록.
3. 검증 담당자: Task 3 (seed 연동 확인) 1회 수행 → 이력 또는 이슈 1줄 기록.
4. (선택) Task 6–8 수행 시 Verification.md·OperatorGuide 등 문서 보강.
