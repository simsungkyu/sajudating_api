...
# Task plan: itemNcard next steps (2026-02-01T074356)
...
- **Task 4 (Review test logic and scope)**: From repo root ran `bash docs/saju/itemNcard/run_verification.sh` (which runs `make test-itemncard` then `go build .`). All tests passed: `types/itemncard`, `service/itemncard`, `service`, `mcplocal`. TestScope.md reviewed; coverage (validation, seed structure, items→tokens, trigger eval, AdminExtractService, mcplocal) confirmed. No gap found; no new test added. Verification.md 검증 이력에 task_2026-02-01T074356 행 추가: "테스트 로직·범위 확인(TestScope.md). Task 6: 자동 검증 완료."
- **Task 6 (Run local automated verification)**: `run_verification.sh` 실행 결과 통과. Verification.md 검증 이력에 위와 동일한 행으로 기록.

**Manual (수동 점검 대기)**

- **Task 1 (PRD §7 checklist (1)–(5))**: 검증 담당자가 Verification.md §7 체크리스트 5항목을 1회 수행 후 각 항목 pass/fail 및 이력 기록 필요.
- **Task 2 (Full-flow check)**: 검증 담당자가 Verification.md 전체 흐름 점검(사주 인생·연도별·월별, 궁합) 1회 수행 후 이력 기록 필요.
- **Task 3 (Seed-folder linkage)**: Seed.md "Seed 파일 참조 현황"과 `docs/saju/itemNcard/seed/` 목록은 이미 정합(9개 시드 옵션·일괄 등록 문서화). 수동 확인(시드에서 불러오기 → 일괄 등록 → 추출 테스트로 선택 카드 반영) 및 "seed 연동 확인 완료" 이력 기록은 검증 담당자 수행 필요.

**Conditional / Optional**

- **Task 5**: Tasks 1–2에서 이슈 기록 없음 → 해당 없음, 스킵.
- **Tasks 7–9**: Optional; 미수행.

**Status**: 자동 검증(Task 4, 6) 완료. §7·전체 흐름·seed 연동 수동 점검 및 옵션(7–9)은 추후 수행.
