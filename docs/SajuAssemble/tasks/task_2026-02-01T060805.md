...
# Task plan: itemNcard next steps (2026-02-01T060805)
...
| Task | Status | Notes |
|------|--------|--------|
| 1. §7 체크리스트 (1)–(5) | 대기 | 검증 담당자 수동 점검; 에이전트 미수행. |
| 2. 전체 흐름 점검 | 대기 | 검증 담당자 수동 점검; 에이전트 미수행. |
| 3. 검증 이력 §7·전체 흐름 기록 | 대기 | Task 1·2 결과 확보 후 Verification.md에 항목별 기록. |
| 4. 검증 발견 이슈 1건 수정 | 해당 없음 | Task 1·2 이슈 미기록. |
| 5. 릴리스 전 자동 검증 | **완료** | `bash -c 'cd api && make test-itemncard'` 및 `go build .` 실행(repo root 기준). 결과: **통과**. itemncard, service, mcplocal 패키지 테스트·빌드 성공. Verification.md 검증 이력에 2026-02-01 행 추가. |
| 6. (선택) 테스트 범위 확인 | **완료** | TestScope.md와 make test-itemncard 대조. make test-itemncard는 `./service/itemncard/...`, `./service/...`, `./mcplocal/...`만 포함; `./types/itemncard/...`(validation_test.go)는 미포함. `go test ./...` 시에는 포함됨. 메모: trigger/score 검증은 별도 `go test ./types/itemncard/...` 또는 전체 `go test ./...`로 실행. |
| 7·8. (선택) E2E 확인 | 미수행 | LLM 컨텍스트 미리보기·y2sl-local MCP 수동 확인. |

**Remaining / next steps**: 검증 담당자 Task 1·2 수행 → Task 3으로 Verification.md 검증 이력에 §7·전체 흐름 결과 기록. (선택) Task 7·8 수동 확인.
