...
# Task plan: itemNcard next steps (2026-02-01T064511)
...
- **Previous plan (064252)**: Task 5 (로컬 자동 검증) 완료. Task 1·2 (PRD §7 체크리스트, 전체 흐름) 수동 점검 대기. Task 4 (seed 연동 확인) 수동 확인 대기. Task 6–8 선택 미수행.
...
**Scope**: Tasks 1–8 from this plan. Agent executed automated parts only; manual verification (Tasks 1, 2, 4, 6–8) was not performed.

| Task | Status | Notes |
|------|--------|--------|
| **1** PRD §7 체크리스트 (1)–(5) | Not executed (manual) | Requires verification lead to run §7 checklist once and record in Verification.md. |
| **2** 전체 흐름 점검 | Not executed (manual) | Requires verification lead to run full flow (saju 인생/연도별/월별, pair) and record. |
| **3** 검증 발견 이슈 1건 수정 | N/A | Conditional on issues from 1·2; no issues recorded by agent. |
| **4** seed 연동 확인 | Not executed (manual) | Requires human to confirm seed files appear in extraction pool; document or one-line issue. |
| **5** 로컬 자동 검증 | **Done** | From repo root: `cd api && make test-itemncard && go build .` (bash -c). **Passed**: types/itemncard, service/itemncard, service, mcplocal; go build succeeded. Verification.md 검증 이력에 1행 추가 (task_2026-02-01T064511). |
| **6–8** (선택) | Not executed | Optional LLM preview, MCP E2E, PRD §6 final check; skipped. |

**Remaining / next steps**

- Verification lead: perform Task 1 (§7 checklist (1)–(5)) and Task 2 (full flow) once; record results in Verification.md (item-wise or one row).
- If issues are found in 1·2, perform Task 3 (fix one issue).
- Optionally: Task 4 (seed linkage), Task 6 (LLM context preview E2E), Task 7 (MCP E2E / runLoopForSaju.sh), Task 8 (PRD §6 final check).
