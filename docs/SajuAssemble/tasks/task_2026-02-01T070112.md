...
# Task plan: itemNcard next steps (2026-02-01T070112)
...
**Executed by**: agent (repo root; `cd api` for Go commands).

| Task | Status | What was done |
|------|--------|----------------|
| **1** | 수동 점검 필요 | PRD §7 체크리스트 (1)–(5) — 검증 담당자 1회 수행 및 Verification.md 이력 기록. 에이전트는 UI 수동 점검 불가. |
| **2** | 수동 점검 필요 | 전체 흐름 점검(사주 인생/연도별/월별, 궁합) — 검증 담당자 수행 및 이력 기록. 에이전트 미실행. |
| **3** | 수동 확인 필요 | seed 폴더·추출 결과 연동 — 검증 담당자 1회 확인 후 이력 또는 이슈 1줄 기록. 에이전트 미실행. |
| **4** | 해당 없음 | Task 1·2 미실행으로 발견 이슈 없음. 1건 수정 생략. |
| **5** | 완료 | `cd api && make test-itemncard && go build .` 성공. **코드 수정**: `api/types/itemncard/validation_test.go` — `TestSeedFileStructure`가 패키지 디렉터리(cwd=api/types/itemncard) 기준으로 동작하도록 경로를 `types/itemncard/testdata` → `testdata`로 변경. Verification.md 검증 이력에 1행 추가(task_2026-02-01T070112). |
| **6** | 미실행 (선택) | LLM 컨텍스트 미리보기 E2E 수동 확인·문서 보강. |
| **7** | 미실행 (선택) | y2sl-local MCP E2E 및 runLoopForSaju.sh 문서화 확인. |
| **8** | 미실행 (선택) | PRD §6 성공 기준 최종 체크 및 이력 반영. |

**Remaining / next steps**

- 검증 담당자: Task 1(§7 체크리스트 5항목), Task 2(전체 흐름), Task 3(seed 연동) 수동 수행 후 Verification.md 검증 이력에 항목별/전체 결과 기록.
- 선택 시 Task 6(LLM 미리보기), Task 7(MCP·runLoopForSaju.sh), Task 8(§6 대조) 수행.
- 로컬 자동 검증: 이미 통과. 재실행 시 `cd api && make test-itemncard && go build .` 권장.
