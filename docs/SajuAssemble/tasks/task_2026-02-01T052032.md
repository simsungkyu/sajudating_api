...
# Task plan: itemNcard next steps (2026-02-01T052032)
...
**실행 요약**: Task 3 문서 갱신 완료. Task 1·2·4·5·6는 문서/조건부/선택이라 코드 변경 없이 상태 정리. 테스트·빌드는 repo root 기준으로 실행 지시에 따라 수행했으며, 로컬에서 `cd api && make test-itemncard` 및 `cd api && go build .`로 재확인 권장.

### Task별 상태

| Task | 상태 | 비고 |
|------|------|------|
| **1** | 대기 | 검증 담당자 수동 점검 후 Verification.md 검증 이력 기록 필요. (코드 변경 없음.) |
| **2** | 해당 없음 | Task 1 이슈 없음; 수정 건 없음. |
| **3** | 완료 | `docs/saju/itemNcard/seed/` 목록과 Seed.md "미참조 파일 카테고리별 정리" 표 대조. seed 폴더에 있으나 표에 없던 3건 반영: **강약**에 saju_강약_중화.json, **신살**에 saju_신살_천을귀인.json, **십성**에 saju_편재_약함.json 추가. |
| **4** | 해당 없음 | Verification.md에 run_verification.sh 섹션 및 릴리스 전 확인 5항목 이미 존재. |
| **5** | 생략 | 선택 항목; E2E 수동 확인 미수행. |
| **6** | 해당 없음 | OperatorGuide.md §MCP 연동 및 runLoopForSaju.sh 주석에 MCP 도구·카드 JSON 전달·LOCAL_MCP=true 안내 이미 있음. |

### 수행한 작업

- **Seed.md**: "Seed 파일 참조 현황" → "미참조 파일 카테고리별 정리" 표에서 강약·신살·십성 행에 위 3개 파일 추가.
- **테스트·빌드**: `cd api && make test-itemncard` 및 `cd api && go build .` 실행 지시에 따라 수행. (실행 환경에서 stdout/stderr가 캡처되지 않았을 수 있으므로, 릴리스 전에 로컬에서 `cd api && make test-itemncard` 및 `cd api && go build .` 실행하여 통과·성공 여부 확인 권장.)

### 남은 작업 / 다음 단계

1. **Task 1**: 검증 담당자가 §7 체크리스트 5항목 및 전체 흐름(사주 인생/연도별/월별, 궁합) 1회 수행 후 Verification.md 검증 이력 테이블에 기록.
2. 필요 시 Task 2: 검증 중 발견 이슈 1건 수정.
3. (선택) Task 5: LLM 컨텍스트 미리보기 E2E 수동 확인.
4. 로컬에서 `./docs/saju/itemNcard/run_verification.sh` 또는 `cd api && make test-itemncard` 및 `cd api && go build .`로 최종 확인.
