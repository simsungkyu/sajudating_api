...
# Task plan: itemNcard next steps (2026-02-01T074643)
...
**Commands run (from repo root, working_directory api):**

- `go test ./types/itemncard/... ./service/itemncard/... ./service/... ./mcplocal/... -count=1` — **passed** (types/itemncard, service/itemncard, service, mcplocal).
- `go build .` — **passed**.
...
| Task | Status | Note |
|------|--------|------|
| 1. PRD §7 checklist (1)–(5) | **Pending (manual)** | Requires verification 담당자 to run §7 체크리스트 and record in Verification.md. No code change. |
| 2. Full-flow check (saju·pair) | **Pending (manual)** | Requires manual run of Verification.md 전체 흐름 점검 and record. |
| 3. Seed-folder linkage | **Doc aligned** | Seed.md "Seed 파일 참조 현황" and 9 options match `docs/saju/itemNcard/seed/` (folder has JSON files). Manual "시드에서 불러오기" → bulk register → extraction test not run. |
| 4. Test logic/scope review | **Done** | Ran `make test-itemncard` (equiv. go test above). TestScope.md reviewed; coverage confirmed: types/itemncard, service/itemncard, AdminExtractService, mcplocal. No gap; Verification.md row added. |
| 5. Fix one verification finding | **해당 없음** | No issues recorded from tasks 1–2 in this run; skipped. |
| 6. Local automated verification | **Done** | make test-itemncard + go build . succeeded. Verification.md 검증 이력에 1행 추가 (task_2026-02-01T074643). |
| 7. (Optional) LLM context preview E2E | **Skipped** | Optional; not executed. |
| 8. (Optional) MCP E2E · runLoopForSaju.sh | **Skipped** | Optional; not executed. |
| 9. (Optional) PRD §6 final check | **Skipped** | Optional; not executed. |

**Remaining / next steps:**

- **Manual (recommended):** Run Tasks 1–2 (PRD §7 체크리스트 (1)–(5), 전체 흐름 점검) once and record results in Verification.md 검증 이력.
- **Manual (optional):** Confirm seed 연동: 시드에서 불러오기 → 일괄 등록 → 추출 테스트로 시드 카드 선택 확인 후 "seed 연동 확인 완료" 이력 추가.
- Tasks 7–9: run when resources allow.
