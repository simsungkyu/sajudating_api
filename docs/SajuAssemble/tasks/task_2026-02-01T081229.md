# Task plan: itemNcard next steps (2026-02-01T081229)
...
- **admweb**: Data Cards page, tabs for 사주 카드 / 궁합 카드 / 사주 추출 테스트 / 궁합 추출 테스트; list (filter/sort/paging); create/edit with seed load and bulk register; PillarDisplay (양=볼드, 오행=컬러); extraction result panels with A/B 명식 요약, selected cards, evidence, LLM 요청 전체 내용 textarea.
...
- **Task 1**: Recorded in Verification.md 검증 이력. §7 체크리스트 (1)–(5) 수동 수행 대기 — 검증 담당자 1회 실행 후 항목별 결과 기록.
- **Task 2**: Recorded in Verification.md. 전체 흐름 점검(사주 인생/연도별/월별, 궁합) 수동 수행 대기 — 1회 점검 후 이력 기록.
- **Task 3**: Recorded in Verification.md. seed 연동 수동 확인 대기 — 시드에서 불러오기→일괄 등록→추출 테스트로 시드 카드 선택 확인 후 "seed 연동 확인 완료" 기록.
- **Task 4**: **통과**. From repo root ran `bash docs/saju/itemNcard/run_verification.sh`. `make test-itemncard` and `go build .` both succeeded (types/itemncard, service/itemncard, service, mcplocal). Added one row to Verification.md 검증 이력.
- **Task 5**: **해당 없음**. No issues recorded from tasks 1–3; skipped.
- **Task 6**: **완료**. Ran `make test-itemncard` (via run_verification.sh). Reviewed TestScope.md coverage; no gap. Added row to Verification.md "테스트 로직·범위 확인 완료" and one paragraph to TestScope.md (task_2026-02-01T081229).
- **Task 7**: **완료**. Consolidated Task 1–4 and 6 results into Verification.md 검증 이력 (one row per task 1–4, 6, 7).

**Status**

- Automated verification: **pass** (make test-itemncard + go build).
- Manual verification: **pending** — §7 체크리스트 (1)–(5), 전체 흐름 점검, seed 연동 확인은 검증 담당자 1회 실행 후 Verification.md에 항목별 결과 기록 필요.

**Remaining / next steps**

1. 검증 담당자: Verification.md §7 체크리스트 (1)–(5) 1회 수행 후 항목별 통과/이슈를 검증 이력에 기록.
2. 검증 담당자: 전체 흐름 점검(사주 인생/연도별/월별, 궁합) 1회 수행 후 결과 기록.
3. 검증 담당자: 시드에서 불러오기→일괄 등록→추출 테스트로 시드 카드 선택 확인 후 "seed 연동 확인 완료" 또는 이슈 1줄 기록.
4. (Optional) Tasks 8–10: LLM context preview E2E, y2sl-local MCP E2E, PRD §6 success criteria final check — resources allowing.
