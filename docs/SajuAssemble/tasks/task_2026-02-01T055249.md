...
# Task plan: itemNcard next steps (2026-02-01T055249)
...
- **Backend**: Card CRUD (GraphQL), soft delete, extraction REST (saju: 인생/연도별/월별, pair). itemncard pipeline: pillars → items → tokens → card selection with evidence; domain/tag limits, cooldown_group, max_per_user. LLM context builder and POST `/api/adm/llm_context_preview`. AdminExtractService returns user/pair info + selected cards with evidence and score. y2sl-local MCP: register_card, update_card, list_cards (mcplocal/cardtools.go).
...
- **Pending**: 검증 담당자에 의한 PRD §7 체크리스트 (1)–(5) 및 전체 흐름 점검 1회 수동 수행 → 검증 이력 기록. (선택) LLM 컨텍스트 미리보기 E2E, y2sl-local MCP E2E. (선택) seed 참조 현황 점검.
...
### 7. seed 폴더 참조 현황 점검 (문서·옵션 대응)

- **목표**: PRD §7 (3) "seed 폴더 내 파일들이 참조되는지 확인"을 문서·기능 수준에서 점검.
- **작업**: Seed.md의 "Seed 파일 참조 현황"과 `docs/saju/itemNcard/seed/` 폴더 내 파일 목록을 대조. admweb "시드에서 불러오기" 옵션(또는 일괄 등록)이 시드 파일과 대응하는지 확인. 불일치·누락이 있으면 Seed.md 또는 OperatorGuide에 1~2문장 보강하거나 이슈 1건 기록.
- **범위**: 문서 대조 + 필요 시 Seed.md/OperatorGuide 1~2문장 또는 이슈 기록.
...
- Task 7: seed 참조와 시드에서 불러오기/일괄 등록 대응 확인; 문서 보강 또는 이슈 기록.
...
| PRD §6 항목 | 대조 결과 |
|-------------|-----------|
| (1) admweb에서 사주 카드·궁합 카드 별도 메뉴로 목록 조회·등록·관리 가능 | 구현됨. 데이터 카드 메뉴, 사주 카드/궁합 카드 탭, CRUD·필터·정렬·페이징·시드에서 불러오기·일괄 등록 등 제공. |
| (2) 사주(인생/연도별/월별)·궁합 추출 테스트 실행 후 결과 화면에서 유저 명식·선택 카드(및 선택 근거) 확인 가능 | 구현됨. Verification.md §2·§7 체크리스트 및 전체 흐름 점검 절차에 따라 검증 담당자 1회 수동 점검 대기. |
| (3) (선택) y2sl-local MCP로 카드 등록 및 runLoopForSaju.sh 연동 가능 | 구현됨. mcplocal/cardtools.go, register_card/update_card/list_cards; OperatorGuide·runLoopForSaju.sh 문서화. |

---

## Execution result (task_2026-02-01T055249)

**Done**

- **Task 1·2**: 수동 점검만; 코드·문서 변경 없음. 검증 담당자 1회 수행 대기.
- **Task 3**: Verification.md 검증 이력 테이블에 행 추가함. "§7·전체 흐름 수동 점검 대기 / task_2026-02-01T055249: Task 3 이력 행 추가. 검증 담당자 Task 1·2 수행 후 항목별 결과를 위 템플릿으로 추가."
- **Task 4**: 해당 없음 (Task 1·2 이슈 미기록).
- **Task 5·6**: 선택 항목; 본 실행에서 생략.
- **Task 7**: Seed.md "Seed 파일 참조 현황"에 **seed/ 폴더 추가 파일** 문단 추가. seed/ 내 9개 옵션·카테고리 표 외 추가 JSON 파일(예: pair_궁합_보완, saju_격국_정인격 등) 존재 가능·일괄 등록으로 불러오기 가능 명시.
- **Task 8**: 본 플랜에 **§6 대조 결과** 표 추가. PRD §6 (1)–(3) 항목별 구현 여부 대조 완료.

**Automated verification**

- `cd api && go test ./...` 및 `go build .` 실행 시 본 환경에서 출력이 캡처되지 않았음. 로컬에서 `cd api && make test-itemncard` 및 `go build .` 실행하여 테스트·빌드 통과 여부 확인 권장.

**Status**

- Task 3, 7, 8 완료. Task 1·2 검증 담당자 수동 점검 후 Verification.md 항목별 결과 기록 필요.

**Next steps**

1. 검증 담당자: Task 1 (PRD §7 체크리스트 (1)–(5)) 1회 수행.
2. 검증 담당자: Task 2 (전체 흐름 점검) 1회 수행.
3. Task 1·2 결과를 Verification.md 검증 이력에 항목별 템플릿으로 기록.
4. (선택) Task 5 LLM 컨텍스트 미리보기 E2E, Task 6 y2sl-local MCP E2E 수동 확인.
