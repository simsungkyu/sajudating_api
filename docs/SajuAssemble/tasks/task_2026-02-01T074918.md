...
# Task plan: itemNcard next steps (2026-02-01T074918)
...
- **Next focus**: PRD §7 checklist and full-flow verification; seed-folder linkage confirmation; test scope review; optional E2E and PRD §6 final check.
...
**Summary**: Automated tasks (4, 6) executed; manual tasks (1, 2, 3) left for verification 담당자. Task 5 해당 없음. Optional tasks 7–9 skipped.

| Task | Status | What was done |
|------|--------|----------------|
| **1** | Not executed (manual) | PRD §7 checklist (1)–(5) requires verification 담당자 to run admweb and record pass/fail per item in Verification.md 검증 이력. |
| **2** | Not executed (manual) | Full-flow check (saju 인생/연도별/월별, pair) requires verification 담당자 to run extraction tests and record result. |
| **3** | Doc confirmed | Seed.md "Seed 파일 참조 현황" aligned with `docs/saju/itemNcard/seed/`: 9 options and seed/ file list match. Manual "시드에서 불러오기 → 등록 → 추출" confirmation remains for verification 담당자. |
| **4** | Done | From repo root ran `bash -c 'cd api && make test-itemncard'`. All tests passed: types/itemncard, service/itemncard, service, mcplocal. TestScope.md reviewed; coverage (validation, seed structure, items→tokens, trigger eval, AdminExtractService, mcplocal) confirmed. No gap found; no new test added. Verification.md 검증 이력에 task_2026-02-01T074918 행 추가(테스트 로직·범위 확인). |
| **5** | 해당 없음 | No issues recorded from tasks 1–2 in this run; skip. |
| **6** | Done | From repo root ran `bash -c 'cd api && make test-itemncard'` (includes `go build .`). Passed. Verification.md 검증 이력에 1행 추가: task_2026-02-01T074918, 통과, make test-itemncard + go build . 성공. |
| **7–9** | Skipped | Optional; not executed. |

**Remaining / next steps**

- Verification 담당자: Run Tasks 1–2 (PRD §7 체크리스트 (1)–(5), 전체 흐름 점검), then add 항목별 결과 to Verification.md 검증 이력.
- Optional: Task 3 manual step (시드에서 불러오기 → 일괄 등록 → 추출 테스트로 선택 카드에 시드 카드 포함 여부 확인); Task 7 (LLM context preview E2E); Task 8 (y2sl-local MCP E2E); Task 9 (PRD §6 성공 기준 이력 대조).
