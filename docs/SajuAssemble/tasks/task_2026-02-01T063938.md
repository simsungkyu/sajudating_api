...
# Task plan: itemNcard next steps (2026-02-01T063938)
...
- **Backend**: Card CRUD (GraphQL), soft delete; extraction REST (saju: 인생/연도별/월별, pair); itemncard pipeline (pillars → items → tokens → card selection with evidence per CardDataStructure/ChemiStructure); LLM context builder and `POST /api/adm/llm_context_preview`; y2sl-local MCP (register_card, update_card, list_cards). Seed structure validation: `TestSeedFileStructure` in `api/types/itemncard/`.
...
- **Previous plan (063648)**: Task 4 (test-itemncard + build) 완료; Task 1·2 (§7 체크리스트, 전체 흐름) 수동 점검 대기; Task 5·6·7 선택 미수행.
...
### 4. 테스트 로직·범위 점검 및 TestScope.md 보강 (PRD §7 마지막 항목)

- **목표**: PRD §7 "테스트는 적당한 로직과 범위로 수행되고 있는지 확인"에 따라 itemNcard 관련 테스트 범위를 1회 점검하고 문서에 반영.
- **작업**: TestScope.md 및 `api/makefile`의 `test-itemncard` 타깃을 확인. `api/service/itemncard/`, `api/service/AdminExtractService_test.go`, `api/types/itemncard/`, `api/mcplocal/cardtools_test.go` 등 테스트가 CardDataStructure(trigger all/any/not, score), TokenRule(items→tokens, where 정규화), ChemiStructure(P_tokens, src P|A|B)와 일치하는지 요약. 누락된 시나리오(예: pair trigger src A/B, 연도별/월별 pillar→items)가 있으면 TestScope.md에 "권장 추가 테스트" 1~3항으로 기록; 이미 적당히 커버되어 있으면 "테스트 범위 점검 완료" 1문단을 TestScope.md 또는 Verification.md 검증 이력에 추가.
- **범위**: 문서 점검 + TestScope.md 또는 Verification.md 1회 업데이트; 필요 시 테스트 1개 추가(선택).

---

### 5. seed 폴더 파일과 추출 결과 연동 확인 (PRD §7 (3) 심화)

- **목표**: seed 폴더 내 JSON이 실제 추출 시 카드 풀에 반영되고, 선택 카드로 등장할 수 있는지 1회 확인.
- **작업**: Seed.md "Seed 파일 참조 현황"과 `docs/saju/itemNcard/seed/` 파일 목록 대조. admweb "시드에서 불러오기"로 1개 이상 시드(예: saju_십성_정재_강함, pair_궁합_충) 불러와 일괄 등록 후, 해당 트리거를 만족하는 생년월일(또는 A/B 쌍)로 추출 테스트 실행. 결과에 해당 카드가 선택되는지 확인. 미반영 시 원인(카드 미등록, trigger 불일치, tokens 생성 규칙) 1줄 문서화; 정상 시 Verification.md 검증 이력에 "seed 연동 확인 완료" 1행 추가.
- **범위**: 수동 확인 + 문서 1행 또는 이슈 1줄; 코드 변경 없음(이슈 발견 시 별도 태스크).

---

### 6. 로컬 자동 검증 실행 및 Verification.md 이력 확인/추가
...
- Task 4: PRD §7 "테스트 적당한 로직과 범위" 확인 → TestScope.md 또는 이력 보강.
- Task 5: seed 파일이 추출 풀에 반영되는지 1회 확인 → 이력 또는 이슈 1줄.
...
- **Task 4 (테스트 로직·범위 점검 및 TestScope.md 보강)**  
  TestScope.md 및 `api/makefile`의 `test-itemncard` 타깃 확인. `api/service/itemncard/`, `AdminExtractService_test.go`, `api/types/itemncard/`, `api/mcplocal/cardtools_test.go`가 CardDataStructure(trigger all/any/not, score), TokenRule(items→tokens, where 정규화, 연도별/월별), ChemiStructure(P_tokens, src P|A|B)와 일치함을 요약. TestScope.md에 **테스트 범위 점검 완료** 1문단 추가(날짜 task_2026-02-01T063938, 권장 추가 테스트 없음).

- **Task 6 (로컬 자동 검증 실행 및 Verification.md 이력)**  
  repo root에서 `go -C api test ./types/itemncard/... ./service/itemncard/... ./service/... ./mcplocal/... -count=1` 및 `go -C api build .` 실행 → **통과**. 추가로 `go -C api test ./... -count=1` 실행 → **통과**. Verification.md 검증 이력 테이블에 1행 추가(날짜 2026-02-01, 결과 통과, 비고 task_2026-02-01T063938).

**Status**

| Task | Status | 비고 |
|------|--------|------|
| 1. PRD §7 체크리스트 (1)–(5) 1회 수행 | 수동 점검 대기 | 검증 담당자 1회 실행 후 Verification.md 항목별 결과 기록 |
| 2. 전체 흐름 점검 (사주 인생/연도별/월별, 궁합) | 수동 점검 대기 | Verification.md 전체 흐름 절차에 따라 실행 후 이력 기록 |
| 3. 검증 발견 이슈 1건 수정 | 해당 없음 | Task 1·2 미수행으로 발견 이슈 없음 |
| 4. 테스트 로직·범위 점검 및 TestScope.md 보강 | 완료 | TestScope.md 테스트 범위 점검 완료 문단 추가 |
| 5. seed 폴더 파일과 추출 결과 연동 확인 | 수동 확인 대기 | 시드 불러오기·일괄 등록 후 추출 테스트로 카드 선택 여부 확인 |
| 6. 로컬 자동 검증 실행 및 Verification.md 이력 | 완료 | go -C api test + build 성공, 검증 이력 1행 추가 |
| 7–9. (선택) LLM 미리보기, MCP E2E, PRD §6 최종 체크 | 미수행 | 리소스 허용 시 수행 |

**Remaining / next steps**

1. **Task 1·2**: 검증 담당자가 PRD §7 체크리스트 (1)–(5) 및 전체 흐름(사주 인생/연도별/월별, 궁합) 1회 수행 후 Verification.md 검증 이력에 항목별/전체 흐름 결과 기록.
2. **Task 5**: seed 폴더와 추출 결과 연동 1회 수동 확인 후, 정상 시 검증 이력 "seed 연동 확인 완료" 1행 추가 또는 미반영 시 원인 1줄 문서화.
3. **Task 7–9**: (선택) LLM 컨텍스트 미리보기 E2E, y2sl-local MCP E2E·runLoopForSaju.sh 문서화, PRD §6 성공 기준 최종 체크 및 이력 반영.
