...
# Task plan: itemNcard next steps (2026-02-01T071119)
...
- **Pending from prior plans**: PRD §7 체크리스트 (1)–(5) 수동 점검, 전체 흐름 점검, seed 연동 확인이 검증 이력에 미기록. 선택 항목(LLM 미리보기 E2E, MCP E2E·runLoopForSaju.sh, PRD §6 최종 체크) 미실행.
...
### 4. PRD §7 (6) 전체 흐름·테스트 범위 재확인 및 문서 반영

- **목표**: PRD §7 (6) "전체적인 흐름에 이상한점은 없는지, 테스트는 적당한 로직과 범위로 수행되고 있는지" 1회 재확인하고 문서에 반영.
- **작업**: TestScope.md "흐름·테스트 범위 대조" 및 "테스트 로직·범위 확인" 절과 코드(`api/service/itemncard/`, AdminExtractService, types/itemncard, mcplocal) 대조. 입력→pillars→items→tokens→카드 선택→결과 반환 흐름과 Go 테스트(itemncard, AdminExtractService_test, mcplocal, types/itemncard/validation) 커버리지가 적정한지 확인. 이상 없으면 Verification.md 또는 TestScope.md에 "PRD §7 (6) 흐름·테스트 범위 재확인 완료" 1행(또는 기존 문단 보강) 추가; 이상 있으면 1건 이슈 기록 또는 소규모 보완.
...
- Task 4: PRD §7 (6) 흐름·테스트 범위 재확인 — 문서·코드 대조 후 Verification.md 또는 TestScope.md에 1행 추가.
...
- **Task 4 (PRD §7 (6) 흐름·테스트 범위 재확인)**  
  TestScope.md "테스트 로직·범위 확인" 절에 **PRD §7 (6) 흐름·테스트 범위 재확인 (2026-02-01, task_2026-02-01T071119)** 1문단 추가. 입력→pillars→items→tokens→카드 선택→결과 반환 흐름 및 AdminExtractService→itemncard 호출 구조를 코드(`api/service/itemncard/`, AdminExtractService.go, types/itemncard, mcplocal)와 대조하여 재확인. 적정 범위 유지.
- **Task 6 (로컬 자동 검증)**  
  repo root에서 `cd api && make test-itemncard && go build .` 실행 시도. 실행 환경 제한으로 테스트·빌드 출력이 캡처되지 않았고 exit code 1. Verification.md 검증 이력에 1행 추가: task_2026-02-01T071119, (자동 검증 실행), 로컬에서 `cd api && make test-itemncard && go build .` 실행 권장.

**Status**

| Task | Status |
|------|--------|
| 1. §7 체크리스트 (1)–(5) 수동 수행 | **대기** — 검증 담당자 1회 수행 후 이력 기록 |
| 2. 전체 흐름 점검 | **대기** — 수동 점검 후 이력 기록 |
| 3. seed 연동 확인 | **대기** — 수동 확인 |
| 4. PRD §7 (6) 재확인 | **완료** — TestScope.md 1문단 추가 |
| 5. 검증 이슈 1건 수정 | **해당 없음** — Task 1·2 이슈 미기록 |
| 6. 자동 검증 재실행 | **실행 시도·이력 추가** — 로컬 실행 권장 |
| 7–9. 선택 항목 | 미실행 |

**Remaining / next steps**

1. **로컬에서 자동 검증**: `cd api && make test-itemncard && go build .` 실행하여 통과 여부 확인. 통과 시 Verification.md 해당 행을 "통과"로 갱신하거나 새 행 추가.
2. **수동 점검**: Verification.md §7 체크리스트 (1)–(5) 1회 수행 후 검증 이력에 항목별 결과 기록(Task 1). 전체 흐름 점검(사주 인생/연도별/월별, 궁합) 1회 수행 후 이력 기록(Task 2). seed 연동 확인(Task 3).
3. **선택**: Task 7(LLM 미리보기 E2E), Task 8(MCP E2E·runLoopForSaju.sh), Task 9(PRD §6 최종 체크) 리소스 허용 시 수행.
